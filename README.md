...
- The number of colors extracted is determined by the user, so that it's dynamic enough for each user
...
4- Right under the color Histogram is a "Color Palette" with each color in a square that when hovered, it shows the RGB value of the color, and if the user clicks on it, it copies the RGB value automatically:
